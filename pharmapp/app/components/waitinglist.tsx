...
export const WaitingList: React.FC<WaitingListProps> = ({
  ...
  const waitingListItems = medicines
    .map((medicine, index) => ({ medicine, index })) // Map each medicine with its original index
    .filter(({ index }) => quantities[index] > 0); // Filter based on quantities using the original index so make sure everything is in with each other.
